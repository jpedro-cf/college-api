...
describe('CreateQuestionCategoryUseCase', () => {
    test('Should throw in use error if repository returns a category in use', async () => {
        ...
        jest.spyOn(questionsCategoryRepository, 'getByField').mockReturnValueOnce(
            Promise.resolve({
                _id: '1232132',
    ...
    test('Should throw if repository throws', async () => {
        const { sut, questionsCategoryRepository } = makeSut()
        jest.spyOn(questionsCategoryRepository, 'getByField').mockReturnValueOnce(Promise.resolve(null))
